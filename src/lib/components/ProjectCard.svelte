...
<div class="bg-primary border rounded-lg p-4 shadow-md hover:shadow-lg text-primary-content h-full">
	...
	/>
	<h2 class="text-xl font-bold mt-4 line-clamp-2">
		<a
	...
	<p class="text-sm mt-2 text-warning line-clamp-1">{project.tagline}</p>
	<p class="text-sm mt-2 truncate line-clamp-2">{project.description}</p>
	<div class="flex items-center justify-between mt-4">
		<div class="flex items-center">
			<img
				src={project.expand?.user
					? `${site.pocketbase}/api/files/${project.expand?.user?.collectionId}/${project.expand?.user?.id}/${project.expand?.user?.avatar}?thumb=100x100`
					: '/logo.png'}
