...
export const GET = async ({ locals, url, cookies }) => {
	const redirectUrl = `${url.origin}/api/oauth/samletnorge`;
	...
	const samletnorgeAuthProvider = authMethods.authProviders.find(
	...
	try {
		await locals.pb
			...
			.authWithOAuth2Code(samletnorgeAuthProvider.name, code, expectedVerifier, redirectUrl, {
				role: ['user']
		...
		console.log('Error Signing up with samletnorge auth', e, e.message);
	}
